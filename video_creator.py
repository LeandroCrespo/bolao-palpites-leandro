...
def get_crespao_image_url(api_key: str) -> str:
    ...
    """
    result = _request("GET", f"{DID_API}/avatars", api_key)
    ...
    for av in avatars:
        ...
        if AVATAR_NAME.lower() in name.lower():
            ...
            if url:
                ...
                return url
            # Tenta buscar detalhes pelo ID
            av_id = av.get("id")
            if av_id:
                detail = _request("GET", f"{DID_API}/avatars/{av_id}", api_key)
...
def create_bulletin_video(script: str, api_key: str, output_path: str = "bulletin.mp4") -> str:
    """
    ...
    while elapsed < POLL_TIMEOUT:
        ...
        status = _request("GET", f"{DID_API}/talks/{talk_id}", api_key)
